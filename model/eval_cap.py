...
def parse_rec_annot(filename, hairmtx, w, h):
...
def parse_rec_det(filename, hairmtx, w, h):
  """ Parse a PASCAL VOC xml file """
  tree = ET.parse(filename)
  objects = []
  for obj in tree.findall("object"):
...
def voc_ap_hair(rec, prec, use_07_metric=True):
...
def voc_eval_hair(detpath, annopath, imagenames, classname, cachedir, 
             roibbox, h, w, ovthresh=0.5, use_07_metric=False):
  ...
  roibbox: Region of interest in image where mAP will be calculated.
      ([xmin, ymin, xmax, ymax]) 
  ...
  for i, imagename in enumerate(imagenames):
    recs_annots[imagename] = parse_rec_annot(os.path.join(annopath, imagename + ".xml"), roibbox, w, h)
  ...
  recs_dets = {}
  for i, imagename in enumerate(imagenames):
    recs_dets[imagename] = parse_rec_det(os.path.join(detpath, imagename + ".xml"), roibbox, w, h)
  ...
  ap = voc_ap_hair(rec, prec, use_07_metric)
  if len(fp) != 0 and len(tp) != 0:
    acc = tp[-1] / (float(npos) + fp[-1])
  else:
    acc = 0.

  return rec, prec, ap, acc